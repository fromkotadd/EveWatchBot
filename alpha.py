...
def greed_trigger(parse_area):
    ...
    pars_string = pytesseract.image_to_string(hsv).replace('\n', ' ').split(' ')

    for word in pars_string:
        if re.search(pattern_1, word):
            teg = re.search(pattern_1, word).group()[1:-1]
            if teg in white_list:
                return False
            else:
                return True
